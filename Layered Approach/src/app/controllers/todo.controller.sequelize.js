...
const service = require('../models/todo.model.sequelize');

const findAll = async (req, res) => {
    try {
        logger.debug('user.controller.sequelize.findAll -> start');
        const result = await service.findAll();
        res.status(200).json({
            data: result
        });
        logger.debug('user.controller.sequelize.findAll -> end');
    } catch (error) {
        logger.error(error);
...
const findById = async (req, res) => {
    try {
        logger.debug('user.controller.sequelize.findById -> start');
